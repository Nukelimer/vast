...
<div class=" relative flex flex-col items-center w-full mx-auto">
	...
	<div class="relative w-full flex items-center justify-center">
		...
		/>
		<div class="select-none absolute bottom-2 left-8  backdrop-blur-3xl text-white w-1/2 md:w-[20%] px-3 py-4 md:py-8 text-sm">
			<p class="text-sm uppercase flex items-center gap-2 font-owners font-semibold mb-6"><span class="h-2 w-2 bg-white"></span> {activeItem.description[0]}</p>
                        <p class="font-owner">{activeItem.description[1]}</p>
		</div>
	</div>

	<div class="thumbnails-container flex flex-col gap-3 select-none absolute right-5 top-[20%] {styles_thumbanail}">
