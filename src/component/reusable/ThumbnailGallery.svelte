...
<div class="gallery-container relative flex flex-col items-center w-full mx-auto">
	...
	<div class="relative w-full flex items-center justify-center">
		...
		/>
		<div class="select-none absolute bottom-2 left-8  backdrop-blur-lg text-white w-1/2 md:w-[20%] px-3 py-4 md:py-8 text-sm">
			<p class="text-sm uppercase font-owners font-semibold mb-6">{activeItem.description[0]}</p>
                        <p class="font-owner ">{activeItem.description[1]}</p>
		</div>
	</div>

	<div class="thumbnails-container flex flex-col gap-3 select-none absolute right-5 top-[20%]">
		{#each items as item (item.id)}
			...
			>
				<img
					...
					class="w-20 md:w-40 md:h-20 min-h-[42px] min-w-[76px] object-cover  cursor-pointer"
				/>
			</div>
		{/each}
	</div>
